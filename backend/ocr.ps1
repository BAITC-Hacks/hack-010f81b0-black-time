...
try {
    ...
    $language = [Windows.Media.Ocr.OcrEngine]::AvailableRecognizerLanguages | Where-Object { $_.LanguageTag -eq 'ru' } | Select-Object -First 1
    $engine = if ($language) { [Windows.Media.Ocr.OcrEngine]::TryCreateFromLanguage($language) } else { [Windows.Media.Ocr.OcrEngine]::TryCreateFromUserProfileLanguages() }
    if (-not $engine) { throw 'OCR language missing' }
    $result = Wait-WinRT ($engine.RecognizeAsync($bitmap)) ([Windows.Media.Ocr.OcrResult])
    [Console]::WriteLine($result.Text)
} catch {
    [Console]::Error.WriteLine('OCR unavailable')
    exit 1
} finally {
